...
const DashboardViewTicketPage: React.FC = () => {
  ...
  const eventStartDate = ticket?.eventStart
    ? new Date(ticket.eventStart)
    : null;

  const eventEndDate = ticket?.eventEnd ? new Date(ticket.eventEnd) : null;

  const formattedEventTime =
    eventStartDate &&
    eventEndDate &&
    isValid(eventStartDate) &&
    isValid(eventEndDate)
      ? `${format(eventStartDate, "Pp")} - ${format(eventEndDate, "Pp")}`
      : "Date not available";
